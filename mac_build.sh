...
cd opensubdiv
git checkout v3_6_0
cmake -S. -Bbuild -DCMAKE_INSTALL_PREFIX=${_LIB_PATH} -DCMAKE_BUILD_TYPE=${_BUILD_TYPE} -GXcode -DNO_PTEX=1 -DNO_DOC=1 -DNO_OMP=1 -DNO_TBB=1 -DNO_CUDA=1 -DNO_OPENCL=1 -DNO_CLEW=1 -DNO_METAL=0 -DGLFW_LOCATION=${_LIB_PATH} -DCMAKE_CXX_FLAGS="-framework Metal -framework Cocoa -framework OpenGL" -DNO_EXAMPLES=1 -DNO_TUTORIALS=1 -DNO_REGRESSION=1 -DPython_EXECUTABLE=${_PYTHON_EXECUTABLE} -DCMAKE_CXX_STANDARD=${_CPP_VERSION}
...
git submodule update --init --recursive 
cmake -S. -Bbuild -DCMAKE_INSTALL_PREFIX=${_LIB_PATH} -DCMAKE_BUILD_TYPE=${_BUILD_TYPE} -DCMAKE_PREFIX_PATH=${_LIB_PATH} -DNANOGUI_BUILD_PYTHON=ON -DMATERIALX_BUILD_PYTHON=ON -DMATERIALX_BUILD_VIEWER=ON -DMATERIALX_BUILD_GRAPH_EDITOR=ON -DPython_EXECUTABLE=${_PYTHON_EXECUTABLE} -DCMAKE_CXX_STANDARD=${_CPP_VERSION}
cmake --build build --config ${_BUILD_TYPE} --target install -j ${_NUM_OF_PROCESSORS}
cd ${_OLD_PATH}
...
cd openusd
git checkout v24.03
cmake -S. -Bbuild -DCMAKE_INSTALL_PREFIX=${_LIB_PATH} -DCMAKE_BUILD_TYPE=${_BUILD_TYPE} -DCMAKE_PREFIX_PATH=${_LIB_PATH} -DTBB_INCLUDE_DIR=${_TBB_INCLUDE_DIR} -DTBB_LIBRARY=${_TBB_LIBRARY} -DBoost_DEBUG=ON -DCMAKE_CXX_STANDARD=${_CPP_VERSION} 
cmake --build build --config ${_BUILD_TYPE} --target install -j ${_NUM_OF_PROCESSORS}
cd ${_OLD_PATH}
